use std::{collections::{HashMap, BTreeSet}, fs};
...
pub fn day07() {
        ...
        match re_name.captures(rule) {
           Some(c1) => {
                for c2 in re_contains.captures_iter(rule) {
                    with_bag.entry(c2[2].to_string())
                        .or_default().push((c2[1].parse::<u32>().unwrap(), c1[1].to_string()));
                    bags_in.entry(c1[1].to_string())
                        .or_default().push((c2[1].parse::<u32>().unwrap(), c2[2].to_string()));
    ...
    let mut search_stack = Vec::<&String>::from_iter(
        with_bag.get("shiny gold").unwrap()
            .iter().map(|(_n, bag)| bag));
    ...
    while ammount_and_bags.len() > 0 {
        ...
        ammount_and_bags.extend(
            bags_in.entry(b).or_default().iter()
